...
from utils.transformations import Point2D, Transformation2D, Transformation3D, TransformationSpherical
from utils.transform import sph2cart, pix2sph, uvgrid, uv2pos, uv2pix
import json
...
class ZillowFloor:
    ...
    def _E2P(self, image, top, bottom, resolution=128, mode='bilinear'):
    ...
    def _P2E(self, tex, image, top, bottom, resolution=256, mode='bilinear'):
        ...
        # vis_torch_im((pano_xyz+5) / 10)
        # vis_torch_im((pano_xyz+1) / 2)
        # pano_xyz[:, 0, ...] = -pano_xyz[:, 0, ...]
        # pano_xyz[:, 1, ...] = -pano_xyz[:, 1, ...]
        print(torch.min(sph[0,1]))
        print(torch.max(sph[0,1]))
        ...
        plt.show()
        # return tex
        # xyz to wall distance
        # p1 = torch.Tensor(p1[None, :, None, None])
        # p1 = torch.cat([p1, torch.zeros_like(p1,[:, 0:1, ...])], dim=1)
        # p2 = torch.Tensor(p2[None, :, None, None])
        # p2 = torch.cat([p2, torch.zeros_like(p2,[:, 0:1, ...])], dim=1)
        # p1 = (p1[None, :, None, None])
        # p2 = (p2[None, :, None, None])
        #
        # pano_xyz_distance = np.cross(p2 - p1, pano_xyz[:, :2, ...].numpy() -p1, axis=1) / np.linalg.norm(p2 - p1, axis=1)
        # # np.cross(p2 - p1, -p1) / np.linalg.norm(p2 - p1)\
        # pano_xyz = pano_xyz * c2w_distance / pano_xyz_distance
        ...
        plt.imshow(valid.float()[0, 0, ..., None] * image/255)
        plt.show()
        # valid = valid & (pano_xyz[:, 0:1, ...] > bottom[0, 0]) & (pano_xyz[:, 1:2, ...] < bottom[1, 0])
        # vis_torch_im(valid.float())
    ...
    def build_tex(self, im_template='panos/{}.jpg', tex_pano=None, pano_im=None, res=128, wall_ids=None, verbose=False, mode='bilinear'):
        # ext = im_template[im_template.rfind('.') + 1:]
        ...
        for wall_id in range(len(floor)):
            ...
            # plt.imshow(tex/255)
            # plt.show()
            # pano_im_reproj = self._P2E(tex, pano_im, ceiling[wall_id], floor[wall_id], resolution=512, mode=mode)
            if verbose:
                plt.imshow(pano_im)
                plt.show()
                plt.imshow(tex)
        ...
        if tex_pano is None:
            # per-room texture
            transform3d = Transformation3D(self.pano_data['ceiling_height'],
                                           camera_height=self.pano_data['camera_height'])
    ...
    def get_wall_id_for_division(self, tex_pano, res):
        # plt.imshow(tex_pano)
        # plt.show()
        walls = self.pano_data['walls']
        ...
        for wall_id in range(len(walls)):
            cur_tex = tex_pano[:, wall_id * res : (wall_id+1) * res, :] > 0
            if np.sum(cur_tex) > 0:
...
def parse_args():
    ...
    parser.add_argument('--raw-path', type=str, default='../data/zind/scenes/layout_merge')
    parser.add_argument('--out-path', type=str, default='../data/zind/scenes/floormesh')
    ...
    parser.add_argument('--hdr', action='store_false')
...
if __name__ == '__main__':
    ...
    split_id, house_id, floor_id, pano_id = opt.split, opt.house, opt.floor, opt.pano
    ...
    #
    wall_tex, floor_tex, ceiling_tex = zf.build_tex(f'{opt.raw_path}/{split_id}/{split_id}+{house_id}+{floor_id}+{pano_id}/color.jpg', tex_pano=None)
    ...
    if opt.hdr:
        wall_tex, floor_tex, ceiling_tex = zf.build_tex(f'../data/zind/scenes/light/{split_id}/{split_id}+{house_id}+{floor_id}+{pano_id}' + '_hdr.exr')
