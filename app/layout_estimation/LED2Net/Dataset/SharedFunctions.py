...
def filter_by_wall_num(rgb_lst, label_lst, wall_types):
    ...
    for i, one in enumerate(label_lst):
        ...
        if label['layoutPoints']['num'] in wall_types:
            new_rgb.append(rgb_lst[i])
            new_label.append(label_lst[i])

    return new_rgb, new_label

def read_image(image_path, shape, use_nearest = False):
    img = imread(image_path, pilmode='RGB').astype(np.float32) / 255

    if img.shape[0] != shape[0] or img.shape[1] != shape[1]: img = cv2.resize(img, dsize=tuple(shape[::-1]), interpolation=cv2.INTER_AREA if not use_nearest else cv2.INTER_NEAREST)
...
def read_label(label_path, cH):
    ...
    xyz_zind = [one['xyz'] for one in label['layoutPoints']['points']]

    # Convert Zind coordinates to LED2Net coordinates
    xyz = [[-xyz_old[0], xyz_old[1], -xyz_old[2]] for xyz_old in xyz_zind]


    # planes = [one['planeEquation'] for one in label['layoutWalls']['walls']]
    # point_idxs = [one['pointsIdx'] for one in label['layoutWalls']['walls']]
    point_idxs = [[i, (i+1) % len(xyz)] for i in range(len(xyz))]
