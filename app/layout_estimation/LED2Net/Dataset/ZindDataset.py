...
class ZindDataset(BaseDataset):
    def __init__(self, dataset_path, mode, shape, image_path, label_path, wall_types, aug, camera_height, **kwargs):
        ...
        # Some panorama dont have layout annotation, thus we obtain the list using label folder
        label_lst = sorted(glob.glob(f'{dataset_path}/{label_path}/*'))
        name_lst = [label.split('/')[-1].replace('_layoutanno.json', '')for label in label_lst]
        rgb_lst = [f'{dataset_path}/{image_path}/{name}.png'for name in name_lst]

        # filter wall type, we may delete it
